...
if not os.path.exists(labfname):
    ...
    if 1:
        seedmap = labimg.get_fdata() > .1
        wseedmap1 = np.where(seedmap)
        wseedmap2 = wseedmap1
    if 1:
        ...
        seedmap = scipy.ndimage.laplace(labimg.get_fdata()) != 0
...
for numid in range(0, 100): # TWEAK: how many image to generate ? (also affects output filename)
    t1= time.time()
    ...
    if 1: # TWEAK: do?
        ws = np.random.randint(0, len(wseedmap1[0]), size=5) # TWEAK: size=How many large deformation blob
        for p in ws:
        ...
        mul_fact = 400. 
        kern = (11, 11, 11) # lower more distorted (5..25)
        canvasx = scipy.ndimage.gaussian_filter(canvasx, kern[0], truncate=3)*mul_fact
        canvasy = scipy.ndimage.gaussian_filter(canvasy, kern[1], truncate=3)*mul_fact
        canvasz = scipy.ndimage.gaussian_filter(canvasz, kern[2], truncate=3)*mul_fact
    ...
    if 1: # TWEAK: do?
        ws = np.random.randint(0, len(wseedmap2[0]), size=50) # TWEAK: size=How many small deformation blob
        for p in ws:
            scales_xyz = np.random.normal(loc=0, scale=1, size=3) # >0 expansion; <0 shrinking
            for k, s, c in zip(kernel2_s, scales_xyz, [canvasx, canvasy, canvasz]):
        ...
        # TWEAK: magnitude of warp effect (need to be adjusted by trial and error)
        mul_fact = 9.
        kern = 5 # the lower, the more distorted
    ...
    if 0: # TWEAK
        ...
        zooms = array([1.,1.,1.,1.])
        zooms[:3] += np.random.normal(scale=.1, size=3) * .05 # TWEAK axis scaling
        angles = np.random.normal(scale=pi/32., size=3) * 0.01 # (.05, .05, .05) # TWEAK: amount of rotation motion on x,y,z
    ...
    # The ANTS command
    # warp the main image:
    os.system ("antsApplyTransforms -d 3 -i %s.nii.gz -r %s.nii.gz  -o generated/%s.nii.gz -t %s_replop1Warp.nii -t %s_replop0Affine.txt --float -n LanczosWindowedSinc" % (subjid, subjid, prefix, prefix, prefix))

    
    #if 0: # TWEAK: warp other similar images, such as labels ?
    ...
    os.system ("antsApplyTransforms -d 3 -i %s_seg.nii.gz -r %s.nii.gz  -o generated/%s_labels.nii.gz -t %s_replop1Warp.nii -t %s_replop0Affine.txt --float -n MultiLabel[0.001] " % (subjid, subjid, prefix, prefix, prefix))


    ##Warp the TSE
    os.system ("antsApplyTransforms -d 3 -i %s_tse.nii.gz -r %s.nii.gz  -o generated/%s_tse.nii.gz -t %s_replop1Warp.nii -t %s_replop0Affine.txt --float -n LanczosWindowedSinc " % (subjid, subjid, prefix, prefix, prefix))
