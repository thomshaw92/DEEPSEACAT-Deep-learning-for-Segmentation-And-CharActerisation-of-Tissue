...
from nipype.interfaces.base import (CommandLineInputSpec, traits, TraitedSpec, File,
...
class C3dInputSpec(CommandLineInputSpec):
    ...
    # Added by the Vikings
    opt_in_file = InputMultiPath(
        File(),
        position=2,
        xor = ["ref_in_file"],
        argstr="%s",
        mandatory=False,
        desc="Optional input file for commands requiring multiple inputs (wildcard and multiple are supported).")
    ref_in_file = InputMultiPath(
        File(),
        xor = ["opt_in_file"],
        position = 2,
        argstr="-popas ref " +"%s",
        mandatory=False,
        desc="Optional ref file meant for popas, when doing multilabel commands, requiring multiple inputs (wildcard and multiple are supported).")
    out_file = File(
        exists=False,
        argstr="-o %s",
        position=-1,
