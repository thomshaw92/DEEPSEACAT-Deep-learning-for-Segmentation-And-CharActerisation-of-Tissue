...
# average all the images in one of the datasets 
for seg in mprage tse ; do
    for side in left right ; do
	AverageImages 3 ${workdir}/${side}_averaged_${seg}.nii.gz 1 ${umc_atlasdir}/train/train*/${seg}_to_chunktemp_${side}.nii.gz

	#fix up the size so you can use the template as a mask
...
for side in left right ; do
	...
	cp ${workdir}/${side}_template/${side}_template0.nii.gz ${workdir}/${side}_tse_template_resampled-0.35mmIso.nii.gz
	cp ${workdir}/${side}_template/${side}_template1.nii.gz ${workdir}/${side}_mprage_template_resampled-0.35mmIso_padded.nii.gz
done
...
#antsApplyTransforms to the affine just created to the segmentation and the native chunk
	...
	antsRegistration --dimensionality 3 --float 0 \  
	--output [${mag_atlasdir}/train0${x}/${x}_${side}_mprage_chunk_resampled-0.35mmIso_,${mag_atlasdir}/train0${x}/${x}_${side}_mprage_chunk_resampled-0.35mmIsoWarped.nii.gz] \  
	--interpolation BSpline \  
	--use-histogram-matching 0 \  
	--transform Rigid[0.15] \  
