...
for participant in {026..060} ; do
    ...
    for x in {0000..0099} ; do
        ...
        mv ./generated/g_mag_mprage_left_v${x}_labels.nii.gz ../0${count}_left/mag_seg_left.nii.gz
        mv ./generated/g_mag_mprage_left_v${x}_tse.nii.gz ../0${count}_left/mag_tse_left.nii.gz
        let "count++"
    done
    rm -r ./generated
done
...
for participant in {026..061} ; do
    ...
    python /30days/uqtshaw/DEEPSEACAT/augment_hippocampus.py mag_mprage_right.nii.gz
    rm mprage_right_tse.nii.gz mprage_right_seg.nii.gz
    #move loop for new segmentations
    for x in {0000..0099} ; do
        mkdir ../0${count}_right
        mv ./generated/g_mag_mprage_right_v${x}.nii.gz ../0${count}_right/mag_mprage_right.nii.gz
        mv ./generated/g_mag_mprage_right_v${x}_labels.nii.gz ../0${count}_right/mag_seg_right.nii.gz
        mv ./generated/g_mag_mprage_right_v${x}_tse.nii.gz ../0${count}_right/mag_tse_right.nii.gz
        let "count++"
    done
    rm -r generated
done
